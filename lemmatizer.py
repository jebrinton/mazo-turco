...
def lemmatize(word, dictionary):
    outputs = findPos(word, dictionary)
    return outputs[0][0].split("_")[0]


def get_lemma_dict():
    with open('Turkish-Lemmatizer/revisedDict.pkl', 'rb') as f:
...
def main():
    ...
    findPos("dinljjkzzfdlskjcektim", revisedDict)

    print(lemmatize("dineljjkzzfdlskjcektim", revisedDict))
